...
| `completion-experiments/ann-classifier-v1` | TBD (run pending) | `extraction-v2-reviewed` | Soros's ANN-similarity + LLM-classifier run on the canonical baseline. Cosine top-k per Concept above threshold, then LLM classifies into the 5-type closed `LINTAS_BUKU_*` vocab from `docs/yhoga-ontology.ttl`. Cross-grade only. JSON-staged then MERGEd into Yhoga via `experiments/replay_completion.py`. See `completion-experiments/ann-classifier-v1/derivation.md`. | TBD (Aura `.backup` taken post-replay) | TBD | First completion experiment on the `extraction-v2-reviewed` baseline. |
